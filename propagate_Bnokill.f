c     *********************************************************
      ...
      real*8 xran,xpold,ypold,rpold,rhopold,zpold,x,y,xmax,fractx
...
c     YO
c     comment this out when going back to tts form of killing
c     photons
      sip=sip*alb
      sqp=sqp*alb
      sup=sup*alb
...
c     YO uncomment
c      if(xran.le.alb) then
          
         iscat=iscat+1

c     YO uncomment
         if (sip.lt.(1.0d-3*alb)) then
            aflux=aflux+1
            go to 400
         endif
...
c     YO uncomment out
c      else
c         if(alb.eq.1.) write(6,*)'error! rlam=1, yet photon absorbed'
c         aflux=aflux+1.
c         go to 400
c     call therm
c      end if
...
c     first, sum fluxes
      rhopold=dsqrt(xpold**2+ypold**2+zpold**2)
      ...
      do ia=1,3
         if (rhopold.lt.aperture(ia)) then
c     flux=flux+1
            si(k,ia)=si(k,ia)+sip
            sq(k,ia)=sq(k,ia)+sqp
            su(k,ia)=su(k,ia)+sup
