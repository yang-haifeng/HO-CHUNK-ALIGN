...
c     rarr(2)=rmine
c     rarr(3)=rddust
c     c1=chiR(1600./11605.)*kappav*rho0*rmincgs**a
c     nrwall=50
c     print*,'c1,a1',c1,a1
c     do ir=4,nrwall
c     tauw=ir*.5
c     rarr(ir)=(tauw*a1/c1+(rddust*rmincgs)**a1)**(1.d0/a1)
c     $        /rmincgs
c     print*,rarr(ir)
c     enddo
         rming1=rarr(nrwall)
...
c     if (gflag.eq.1.or.gflag.eq.2) then
c     call locate(rarr,nrg,rming2,ir)
c     rarr(ir+1)=rming2
c     endif
...
c     set up diffusion grid
      if (massd.eq.0.d0) then
         ...
         do ir=nr0,nrg-1
            r=0.5*(rarr(ir)+rarr(ir+1))/rddust
            sigmu=mu0*r**(b-1.)
            Rsonr=1./(rddust*r)
            Tdisk=min(1600./11605.,max(3./11605.,(Tstar/11605.)*(
     +           max(2./3.*(Rsonr)**3,
     +           (asin(Rsonr)-(Rsonr)*sqrt(1.-(Rsonr)**2)))/pi
c     +        +   (3.*Lacc*(Rsonr)**3*(1.-sqrt(Rsonr)))
     +           )**0.25))
            taumid=tauzrat*tauv*chiR(sngl(Tdisk))/r**(a-b)
            if (taumid.gt.taudisk) then
               mudisk=sigmu*sqrt(2.)*ierfc(sngl(taudisk/taumid))
            else
               mudisk=0.
            endif
...
c     calculate density in grid
     ...
     1                 ,dense)
            ...
            ip=1
            it=1
            tdust(ir,it,ip)=1000.d0/11605.d0
            nabs(ir,it,ip)=0
...
c     TSC blows up at thet=90
         ...
         do ir=1,nrg-1
            ...
            call densenv(rad,thet,cost,sint,pi,pihalf,phi,dense)
            call densdisk(rad,sint,cost,phi,densd)
            taud=taud+kapd*(densd)*dr
            taue=taue+kapd*(dense)*dr
         end do
...
c     call densenv(rad,thet,cost,sint,pi,pihalf,phi,dense)
         call densdisk(rad,sint,cost,phi,densd)
         taud=taud+kappav*rmincgs*(densd)*dr
...
c     calculate Av along ethet,ephi directions (peeled image)
     ...
     1 'theta   Av_env       Av_disk      Av_tot'
      ...
      do ir=1,nrg-1
         ...
         call densdisk(rad,sint,cost,phi,densd)
         taud=taud+kappav*rmincgs*(densd)*dr
         taue=taue+kappav*rmincgs*(dense)*dr
      end do
